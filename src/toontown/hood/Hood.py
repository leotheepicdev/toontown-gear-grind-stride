...
class Hood(StateData.StateData):
    ...
    def considerReloadStreetSky(self, zoneId):
        if zoneId in self.SUBZONE_2_SKY:
            self.stopSky()
            self.sky.removeNode()
            self.sky = None
            self.sky = loader.loadModel(self.SUBZONE_2_SKY[zoneId])
            self.activeSky = self.SUBZONE_2_SKY[zoneId]
            self.sky.setTag('sky', 'Regular')
            self.sky.setScale(1)
            self.sky.setFogOff()
            if self.activeSky == 'phase_9/models/cogHQ/cog_sky':
    ...
    def considerReloadPlaygroundSky(self):
        if self.activeSky != self.skyFile:
            self.stopSky()
            self.sky.removeNode()
            self.sky = None
            self.createDefaultSky()
        
    def createDefaultSky(self):
        if not base.cr.newsManager.isHolidayRunning(ToontownGlobals.HALLOWEEN) or not self.spookySkyFile:
            self.sky = loader.loadModel(self.skyFile)
            self.activeSky = self.skyFile
            self.sky.setTag('sky', 'Regular')
