from panda3d.core import Lens, Point3, TextNode, Vec4
...
class DistributedAvatar(DistributedActor, Avatar):
    ...
    def takeDamage(self, hpLost, bonus = 0, crit = 0):
        ...
        if hpLost > 0:
            self.showHpText(-hpLost, bonus=bonus, crit=crit)
            self.hpChange(quietly=0)
    ...
    def showHpText(self, number, bonus = 0, scale = 1, crit = 0):
    ...
    def showHpString(self, text, duration = 0.85, bonus=-1, scale = 0.7):
        if self.HpTextEnabled and not self.ghostMode:
            if text != '':
                if self.hpText:
                    self.hideHpText()
