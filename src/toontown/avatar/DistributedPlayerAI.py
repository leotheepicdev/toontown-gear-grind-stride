from panda3d.core import Datagram, Notify
...
def system(message):
    ...
    dg = dclass.aiFormatUpdate('systemMessage',
                               ToontownDoGlobals.DO_ID_CLIENT_SERVICES_MANAGER,
                               10, 1000000, [message])
    simbase.air.send(dg)
...
def maintenance(minutes):
    ...
    def disconnect(task):
        ...
        dg.addUint16(105)
        dg.addString('Toontown: Gear Grind is now closed for maintenance.')
        simbase.air.send(dg)
        return Task.done
...
def stopMaintenance():
    ...
    system(OTPLocalizer.CRCancelMaintenanceMessage)
