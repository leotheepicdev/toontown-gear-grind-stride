...
class FriendsListPanel(DirectFrame, StateData.StateData):
    ...
    def __init__(self):
        ...
        self.rightmostPanel = FLPAccountToons
    ...
    def makeAccountToonButton(self, avId, name, color):
        return DirectButton(relief=None, text=name, text_scale=0.04, text_align=TextNode.ALeft, text_fg=color, text_shadow=None, text1_bg=self.textDownColor, text2_bg=self.textRolloverColor, text3_fg=self.textDisabledColor, text_font=ToontownGlobals.getToonFont(), textMayChange=0, command=self.__choseFriend, extraArgs=[avId])
    ...
    def createAccountToonButtons(self, avs, nametag):
        for av in avs:
            if av.id == base.localAvatar.doId:
                continue
            if av.id not in self.friends:
                button = self.makeAccountToonButton(av.id, av.name, nametag)
                if button:
                    self.scrollList.addItem(button, refresh=0)
                    self.friends[av.id] = button
    ...
    def __updateScrollList(self):
        ...
        if self.panelType == FLPAccountToons:
            self.createAccountToonButtons(base.cr.avList, NAMETAG_COLORS[CCSpeedChat][0][0])
    ...
    def __updateTitle(self):
        if self.panelType == FLPOnline:
            ...
            self.title['text'] = TTLocalizer.FriendsListPanelAccountToons
