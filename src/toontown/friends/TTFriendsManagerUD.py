...
class OperationFSM(FSM):
    ...
    def __init__(self, mgr, air, senderAvId, senderAccId=None, targetAvId=None, callback=None):
        FSM.__init__(self, 'OperationFSM-%s' % senderAvId)
        self.mgr = mgr
        self.air = air
        self.sender = senderAvId
        self.senderAcc = senderAccId
        self.result = None
...
class FriendsListOperation(OperationFSM):
    ...
    def handleRetrieveSender(self, dclass, fields):
        if dclass != self.air.dclassesByName['DistributedToonUD']:
            self.mgr.killConnection('handleRetrieveSender: Distributed Class was not a Toon.', self.sender)
    ...
    def retrieveAccountFriends(self):
        self.air.dbInterface.queryObject(self.air.dbId, self.senderAcc, self.handleRetrieveAccountFriends)
        
    def handleRetrieveAccountFriends(self, dclass, fields):
        if dclass != self.air.dclassesByName['AccountUD']:
            self.mgr.killConnection('handleRetrieveAccountFriends: Distributed Class was not an account.', self.sender)
            self.demand('Error', 'Distributed Class was not an account.')
            return
        self.accountFriendIndex = 0
        self.avList = fields['ACCOUNT_AV_SET']
        self.avList = self.avList[:6]
        self.avList = [x for x in self.avList if x != 0]
        self.avList.remove(self.sender)
        if len(self.avList) == 0:
            self.demand('Off')
            return
        self.air.dbInterface.queryObject(self.air.dbId, self.avList[self.accountFriendIndex], self.addAccountFriend)
            
    def addAccountFriend(self, dclass, fields):
        if dclass != self.air.dclassesByName['DistributedToonUD']:
            self.mgr.killConnection('addAccountFriend: Distributed Class was not a Toon.', self.sender)
            self.demand('Error', 'Distributed Class was not a Toon.')
            return
        self.realFriendsList.append([self.avList[self.accountFriendIndex], fields['setName'][0], fields['setDNAString'][0], fields['setAdminAccess'][0], fields['setPetId'][0]])
        if self.accountFriendIndex >= len(self.avList) - 1:
            self.result = self.realFriendsList
            self.demand('Off')
            return
        self.accountFriendIndex += 1
        self.air.dbInterface.queryObject(self.air.dbId, self.avList[self.accountFriendIndex], self.addAccountFriend)

    def addFriend(self, dclass, fields):
        if dclass != self.air.dclassesByName['DistributedToonUD']:
            self.mgr.killConnection('addFriend: Friend was not a toon', self.sender)
...
class RemoveFriendOperation(OperationFSM):
    ...
    def __init__(self, mgr, air, senderAvId, targetAvId=None, callback=None, alert=False):
        OperationFSM.__init__(self, mgr, air, senderAvId, None, targetAvId, callback)
        self.alert = alert
    ...
    def handleRetrieve(self, dclass, fields):
        if dclass != self.air.dclassesByName['DistributedToonUD']:
            self.mgr.killConnection('handleRetrieve: Distributed Class was not a Toon.', self.sender)
            self.demand('Error', 'Distributed Class was not a Toon.')
            return
...
class ClearListOperation(OperationFSM):
    ...
    def handleRetrieved(self, dclass, fields):
        if dclass != self.air.dclassesByName['DistributedToonUD']:
            self.mgr.killConnection('handleRetrieved: Distributed Class was not a Toon.', self.sender)
            self.demand('Error', 'Distributed Class was not a Toon.')
            return
        self.demand('Retrieved', fields['setFriendsList'][0])
...
class TTFriendsManagerUD(DistributedObjectGlobalUD):
    ...
    def requestFriendsList(self):
        ...
        newOperation = FriendsListOperation(self, self.air, avId, accId,
    ...
    def teleportResponse(self, toId, available, shardId, hoodId, zoneId):
        ...
        if self.tpRequests.get(toId) != fromId:
            ...
            self.mgr.killConnection('Invalid teleportResponse. Contact a developer.', self.sender) 
            return
    ...
    def killConnection(self, reason, av):
        self.notify.warning('Killing connection(%s): %s' % (av, reason))
        dg = PyDatagram()
