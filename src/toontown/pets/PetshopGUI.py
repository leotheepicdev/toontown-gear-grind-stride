...
class PetshopGUI(DirectObject):
    ...
    class MainMenuDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.cancelButton = DirectButton(parent=self, relief=None, clickSound=loader.loadSfx('phase_3/audio/sfx/GUI_create_toon_back.ogg'), scale=(modelScale, modelScale, modelScale), geom=XImageList, image=cancelImageList, text=('', TTLocalizer.PetshopCancel), text_pos=TTLocalizer.PGUIcancelButtonPos, text_scale=0.8, pressEffect=False, command=lambda : messenger.send(doneEvent, [0]))
    ...
    class AdoptPetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, petNameIndex):
            ...
            self.cancelButton = DirectButton(parent=self, relief=None, clickSound=loader.loadSfx('phase_3/audio/sfx/GUI_create_toon_back.ogg'), image=cancelImageList, geom=cancelIcon, scale=modelScale, text=('', TTLocalizer.PetshopGoBack), text_pos=(-5.8, 4.4), text_scale=0.7, pressEffect=False, command=lambda : messenger.send(doneEvent, [0]))
    ...
    class ChoosePetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeeds):
            ...
            self.cancelButton = DirectButton(parent=self, relief=None, clickSound=loader.loadSfx('phase_3/audio/sfx/GUI_create_toon_back.ogg'), image=cancelImageList, geom=cancelIcon, scale=modelScale, pressEffect=False, command=lambda : messenger.send(doneEvent, [-1]))
