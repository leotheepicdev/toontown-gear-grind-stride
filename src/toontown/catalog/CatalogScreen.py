from panda3d.core import Camera, DisplayRegion, Lens, NodePath, PerspectiveLens, Plane, PlaneNode, Point3, TextNode, Vec3, Vec4
...
class CatalogScreen(DirectFrame):
    ...
    def __makeFFlist(self):
        for id, handle in base.cr.friendsMap.items():
            self.ffList.append((id, handle.getName(), NametagConstants.CCNormal))

    def __makeScrollList(self):
        for ff in self.ffList:
            ffbutton = self.makeFriendButton(ff[0], ff[1], ff[2])
            if ffbutton:
