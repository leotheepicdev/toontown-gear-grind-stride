...
from toontown.suit.SuitInvasionGlobals import IFSkelecog, IFWaiter, IFV2, IFExecutive

# TODO: MASSIVE CLEANUP

class CogHQBossPlannerAI:
    notify = DirectNotifyGlobal.directNotify.newCategory('CogHQBossPlannerAI')
    ...
    def _genSuitInfos(self, numFloors, bldgLevel, bldgTrack):
        ...
        for currFloor in xrange(numFloors):
            ...
            bldgInfo = SuitBuildingGlobals.CogBosses[bldgLevel]
    ...
    def __genSuitObject(self, suitZone, suitType, bldgTrack, suitLevel, revives = 0, virtual = 0):
        newSuit = DistributedSuitAI.DistributedSuitAI(simbase.air, None)
        self.__setupSuitInfo(newSuit, bldgTrack, suitLevel, suitType)
        newSuit.setSkeleRevives(revives)
        newSuit.generateWithRequired(suitZone)
        if not newSuit.getSkelecog():
            if virtual:
                if random.random() <= 0.1:
                    newSuit.b_setSkelecog(1)
                    newSuit.b_setVirtual((1, random.randint(0, 4)))					
    ...
    def genFloorSuits(self, floor):
        ...
        for activeSuitInfo in floorInfo['activeSuits']:
            suit = self.__genSuitObject(self.zoneId, activeSuitInfo['type'], activeSuitInfo['track'], activeSuitInfo['level'], activeSuitInfo['revives'], activeSuitInfo['virtual'])
            activeSuits.append(suit)
        suitHandles['activeSuits'] = activeSuits
        reserveSuits = []
        for reserveSuitInfo in floorInfo['reserveSuits']:
            suit = self.__genSuitObject(self.zoneId, reserveSuitInfo['type'], reserveSuitInfo['track'], reserveSuitInfo['level'], reserveSuitInfo['revives'], reserveSuitInfo['virtual'])
