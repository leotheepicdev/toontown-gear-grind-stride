...
class DistributedMintRoom(DistributedLevel.DistributedLevel, MintRoomBase.MintRoomBase, MintRoom.MintRoom):
    ...
    def getFloorOuchLevel(self):
        mintRoom = self.roomId # Toon area id (See MintRoomSpecs or press f2 in the mint in whatever location)
        if mintRoom == 10: # Sinking platform area
            self.ouchSound = base.loadSfx('phase_9/audio/sfx/CHQ_FACT_lava_fall_in.ogg')
            base.playSfx(self.ouchSound, volume=0.5) # Play ouchSound
        elif mintRoom == 13: # Moving platform area
            self.ouchSound = base.loadSfx('phase_9/audio/sfx/CHQ_FACT_paint_splash.ogg')
            base.playSfx(self.ouchSound, volume=0.5) # Play alternate ouchSound
        return 8
