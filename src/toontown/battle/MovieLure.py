from panda3d.core import Plane, PlaneNode, Point3, Point4, Vec3, Vec4
...
def __createFishingPoleMultiTrack(lure, dollar, dollarName):
    ...
    if sidestep == 0:
        if kbbonus == 1 or hp > 0:
            ...
            opos.setY(opos.getY() + MovieUtil.SUIT_EXTRA_REACH_DISTANCE)
            if suitName in MovieUtil.largeSuits:
                moveTrack = lerpSuit(suit, 0.0, reachAnimDuration / 2.5, opos, battle, trapProp)
