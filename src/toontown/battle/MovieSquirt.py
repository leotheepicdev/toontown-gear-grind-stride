from panda3d.core import GeomNode, Point3, Point4, VBase3
...
def doSquirts(squirts):
    ...
    for st in suitSquirts:
        if len(st) > 0:
            ival = __doSuitSquirts(st, npcs)
            if ival:
                mtrack.append(Sequence(Wait(delay), ival))
            delay = delay + TOON_SQUIRT_SUIT_DELAY
    retTrack = Sequence(npcArrivals, mtrack, npcDepartures)
    camDuration = mtrack.getDuration()
    enterDuration = npcArrivals.getDuration()
    exitDuration = npcDepartures.getDuration()
    camTrack = MovieCamera.chooseSquirtShot(squirts, suitSquirtsDict, camDuration, enterDuration=enterDuration, exitDuration=exitDuration)
    return (retTrack, camTrack)


def __doSuitSquirts(squirts, npcs):
    uberClone = 0
    toonTracks = Parallel()
    delay = 0.0
    if type(squirts[0]['target']) == type([]):
        ...
        fShowStun = 0
    for s in squirts:
        tracks = __doSquirt(s, delay, fShowStun, uberClone, npcs)
...
def __doSquirt(squirt, delay, fShowStun, uberClone=0, npcs=[]):
    squirtSequence = Sequence(Wait(delay))
    if 'npc' in squirt:
        squirt['toon'] = squirt['npc']
    if type(squirt['target']) == type([]):
        ...
        notify.debug('toon: %s squirts prop: %d at suit: %d for hp: %d' % (squirt['toon'].getName(),
         ...
         squirt['target']['hp']))
    if uberClone:
        ival = squirtfn_array[squirt['level']](squirt, delay, fShowStun, uberClone, npcs=npcs)
        if ival:
            squirtSequence.append(ival)
    else:
        ival = squirtfn_array[squirt['level']](squirt, delay, fShowStun, npcs=npcs)
        if ival:
            squirtSequence.append(ival)
...
def __getSuitTrack(suit, tContact, tDodge, hp, hpbonus, kbbonus, anim, died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun = 0.5, afterStun = 1.8, geyser = 0, uberRepeat = 0, revived = 0, npcs=[]):
    if hp > 0:
        suitTrack = Sequence()
...
def __doFlower(squirt, delay, fShowStun, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    ...
    if hp > 0 or delay <= 0:
        tracks.append(__getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'squirt-small-react', died, leftSuits, rightSuits, battle, toon, fShowStun, revived=revived, npcs=npcs))
    return tracks


def __doWaterGlass(squirt, delay, fShowStun, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    hpbonus = squirt['hpbonus']
    target = squirt['target']
...
def __doWaterGun(squirt, delay, fShowStun, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    hpbonus = squirt['hpbonus']
    target = squirt['target']
    ...
    tSuitDodges = 1.1
    tracks = Parallel()
    toonTrack = Sequence(Func(toon.headsUp, battle, suitPos), ActorInterval(toon, 'water-gun'), Func(toon.loop, 'neutral'))
    if not 'npc' in squirt:
        toonTrack.append(Func(toon.setHpr, battle, origHpr))
    tracks.append(toonTrack)
...
def __doSeltzerBottle(squirt, delay, fShowStun, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    hpbonus = squirt['hpbonus']
    target = squirt['target']
    ...
    tracks = Parallel()
    toonTrack = Sequence(Func(toon.headsUp, battle, suitPos), ActorInterval(toon, 'hold-bottle'), Func(toon.loop, 'neutral'))
    if not 'npc' in squirt:
        toonTrack.append(Func(toon.setHpr, battle, origHpr))
    tracks.append(toonTrack)
...
def __doFireHose(squirt, delay, fShowStun, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    hpbonus = squirt['hpbonus']
    target = squirt['target']
    ...
    tSuitDodges = 2.1
    tracks = Parallel()
    toonTrack = Sequence(Wait(tAppearDelay), Func(toon.headsUp, battle, suitPos), ActorInterval(toon, 'firehose'), Func(toon.loop, 'neutral'))
    if not 'npc' in squirt:
        toonTrack.append(Func(toon.setHpr, battle, origHpr))
    tracks.append(toonTrack)
    ...
    if hp > 0 or delay <= 0:
        tracks.append(__getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'squirt-large-react', died, leftSuits, rightSuits, battle, toon, fShowStun, revived=revived, npcs=npcs))
    return tracks


def __doStormCloud(squirt, delay, fShowStun, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    hpbonus = squirt['hpbonus']
    target = squirt['target']
    ...
    if hp > 0 or delay <= 0:
        tracks.append(__getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'soak', died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun=2.6, afterStun=2.3, revived=revived, npcs=npcs))
    return tracks


def __doGeyser(squirt, delay, fShowStun, uberClone=0, npcs=[]):
    toon = squirt['toon']
    level = squirt['level']
    hpbonus = squirt['hpbonus']
    tracks = Parallel()
    ...
    for target in squirt['target']:
        ...
        if hp > 0 or delay <= 0:
            tracks.append(Sequence(Wait(delayTime), __getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'soak', died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun=2.6, afterStun=2.3, geyser=1, uberRepeat=uberClone, revived=revived, npcs=npcs)))
