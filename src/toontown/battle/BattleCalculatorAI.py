...
class BattleCalculatorAI:
    ...
    def __init__(self, battle):
        ...
        self.propBonusRounds = 0
        self.doodlesPowerRounds = 0
        self.bonusPowerUp = 1
    ...
    def __targetDefense(self, suit, atkTrack):
        ...
        if suit.getExecutive():
            bonus = EXECUTIVE_DEF_BONUS
        elif suit.getVirtual() == VIRTUAL_DEF:
            bonus = VIRTUAL_DEF_BONUS
        elif suit.getVirtual() == VIRTUAL_LUCK:
            if random.random() >= VIRTUAL_DEF_CHANCE:
                bonus = VIRTUAL_DEF_BONUS
        suitDef = SuitBattleGlobals.getSuitACCDefense(SuitBattleGlobals.SuitAttributes[suit.dna.name]['level'], suit.getActualLevel(), bonus)
        return -suitDef
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if atkTrack == LURE:
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    suit = self.battle.findSuit(targetId)
                    if atkTrack != HEAL:
                        defenseDebuff, defenseMaxedOut = suit.getDefenseDebuff()
                        if defenseDebuff > 0:
                            attackDamage = int(math.ceil(attackDamage - (attackDamage * defenseDebuff)))
                if not self.__combatantDead(targetId, toon=toonTarget):
    ...
    def __suitIsSkelecog(self, suitId):
        suit = self.battle.findSuit(suitId)
        return suit.getSkelecog()

    def __suitIsExecutive(self, suitId):
        suit = self.battle.findSuit(suitId)
        return suit.getExecutive()
        ...
        for currTgt in bonusList:
            for currAtkType in currTgt.keys():
                if len(currTgt[currAtkType]) > 1 or not hp and len(currTgt[currAtkType]) > 0:
                    ...
                    attack = self.battle.toonAttacks[attackerId]
                    if hp:
                        if self.bonusPowerUp:
                            attack[TOON_HPBONUS_COL] = math.ceil(totalDmgs * (self.SOSDamageBonuses[numDmgs - 1] * 0.01))
                        else:
                        ...
                        if self.bonusPowerUp:
                            attack[TOON_KBBONUS_COL][tgtPos] = totalDmgs
                        else:
    ...
    def __calcSuitTarget(self, attackIndex):
        ...
        if len(self.battle.activeToons) == 1:
            return 0
        if self.__suitIsExecutive(suitId):
            if random.random() <= 0.5:
                return self.__pickLowestHpToon(suitId)
        if self.__suitIsSkelecog(suitId):
    ...
    def __pickLowestHpToon(self, suitId):
        liveToons = []
        hpAmounts = []
        for currToon in self.battle.activeToons:
            if not self.__combatantDead(currToon, toon=1):
                liveToons.append(self.battle.activeToons.index(currToon))
                hpAmounts.append(self.__getToonHp(currToon))
        if len(liveToons) == 0:
            return -1
        lowestHpId = hpAmounts.index(min(float(i) for i in hpAmounts))
        chosen = liveToons[lowestHpId]
        return chosen
    ...
    def __suitAtkHit(self, attackIndex):
        ...
        if self.suitsAlwaysMiss:
            return 0
        theSuit = self.battle.activeSuits[attackIndex]
        if theSuit.getVirtual() == VIRTUAL_ACC:
            return 1
        if theSuit.getVirtual() == VIRTUAL_LUCK:
            if random.random() >= VIRTUAL_ACC_CHANCE:
                return 1
        ...
        if atkInfo['name'] in ('CogUp', 'Barrier'):
            return 1
        atkAcc = atkInfo['acc']
        randChoice = random.randint(0, 99)
        if self.notify.getDebug():
            self.notify.debug('Suit attack rolled ' + str(randChoice) + ' to hit with an accuracy of ' + str(atkAcc))
        if randChoice < atkAcc:
            return 1
        return 0
    ...
    def __calcSuitAtkHp(self, attackIndex):
        ...
        theSuit = self.battle.findSuit(attack[SUIT_ID_COL])
        if not theSuit:
            # Hmm... Cog doesn't exist. So we don't crash, don't bother doing anything else.
            return
        crit = 0
        virtualLuck = theSuit.getVirtual() == VIRTUAL_LUCK
        if virtualLuck:
            critChance = 0.4
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if (toon and toon.immortalMode) or self.TOONS_TAKE_NO_DAMAGE:
                ...
                if difficulty == DistrictGlobals.DIFFICULTY_HARD: 
                    bonus += HARD_DAMAGE_BONUS
                elif difficulty == DistrictGlobals.DIFFICULTY_EXTREME:
                    bonus += EXTREME_DAMAGE_BONUS
                if theSuit.getExecutive():
                    bonus += EXECUTIVE_DAMAGE_BONUS
                elif theSuit.getVirtual() == VIRTUAL_ATK:
                    bonus += 1
                if atkInfo['name'] == 'CogUp':
                    for suit in self.battle.activeSuits:
                        if self.__combatantDead(suit.doId, toon=0): # make sure the cog isn't dead so we don't have any bugs
                            continue
                        suit.b_setHP(min(suit.getMaxHP(), suit.getHP() + int(math.ceil(suit.getMaxHP() / 2))))
                elif atkInfo['name'] == 'Barrier':
                    for suit in self.battle.activeSuits:
                        if self.__combatantDead(suit.doId, toon=0): # make sure the cog isn't dead so we don't have any bugs
                            continue
                        defDebuff, defenseMaxedOut = suit.getDefenseDebuff() # we include a maxed out value so our healing after reaching max defense works properly.
                        if not defenseMaxedOut:
                            if Decimal(defDebuff) == Decimal(MAX_SUIT_DEFENSE):
                                suit.b_setDefenseDebuff(defDebuff, defenseMaxedOut=1) # let the game know the cog maxed out their defense so the movie works properly.
                                suit.b_setHP(min(suit.getMaxHP(), suit.getHP() + int(math.ceil(suit.getMaxHP() / 2))))
                            else:
                                suit.b_setDefenseDebuff(defDebuff + 0.15, defenseMaxedOut=0)
                        else:
                            suit.b_setHP(min(suit.getMaxHP(), suit.getHP() + int(math.ceil(suit.getMaxHP() / 2))))
                if crit:
                    bonus += CRIT_DAMAGE_BONUS
                if virtualLuck:
                    result = int(math.ceil(atkInfo['hp'] * bonus)) + random.randint(*VIRTUAL_LUCK_DMG_RANGE)
                else:
                    result = int(math.ceil(atkInfo['hp'] * bonus))
                if atkInfo['name'] == 'SongAndDance':
                    if not self.__combatantDead(theSuit.doId, toon=0):
                        theSuit.b_setHP(min(theSuit.getMaxHP(), theSuit.getHP() + (result * 2)))
            targetIndex = self.battle.activeToons.index(toonId)
            attack[SUIT_HP_COL][targetIndex] = result
            attack[SUIT_CRIT_COL] = crit
    ...
    def __initRound(self):
        ...
        for special in specials:
            ...
            if npc_track == NPC_TOONS_HIT:
                ...
                cogsMiss = 1
            elif npc_track == NPC_ORGANIC_GAGS:
                self.propBonusRounds = rounds
            elif npc_track == NPC_DOODLES_POWER:
                self.doodlesPowerRounds = rounds
            elif npc_track == NPC_BONUS_POWER:
                self.bonusPowerUp = 1
    ...
    def calculateRound(self):
        ...
        for suit in self.battle.activeSuits:
            if not hasattr(suit, 'dna'):
                ...
                self.battle.activeSuits.remove(suit)
                continue
            if suit.isGenerated():
                suit.b_setHP(suit.getHP())
        if not self.battle.activeSuits:
            self.notify.warning('calculateRound(): no active suits are in this battle!')
            # Instead of stalling out the battle, simply abort the battle.
            self.battle.abortBattle()
            return
        ...
        if self.propBonusRounds > 0:
            self.propBonusRounds -= 1
        if self.doodlesPowerRounds > 0:
            self.doodlesPowerRounds -= 1
        if self.bonusPowerUp == 1:
            self.bonusPowerUp = 0
        if self.notify.getDebug():
