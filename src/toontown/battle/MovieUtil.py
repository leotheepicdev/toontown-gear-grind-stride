from panda3d.core import Point3, Vec3, Vec4, lookAt
...
def hideDeathSuit(deathSuit):
    notify.debug('hideDeathSuit()')
    if deathSuit:
        for node in ('legs', 'hands', 'torso', 'arms', 'Lhand', 'Rhand'):
            node = deathSuit.find('**/%s' % node)
...
def createSuitDeathTrack(suit, toon, battle, npcs = []):
    ...
    suitTrack.append(Parallel(ActorInterval(deathSuit, 'lose', duration=SUIT_LOSE_DURATION)))
...
def indicateDefenseRise(actor, value=15, duration = 1.1, bonus=3, scale = 0.7):
    actor.showHpString(TTLocalizer.DefenseRisen % value, duration=duration, bonus=bonus, scale=scale)
