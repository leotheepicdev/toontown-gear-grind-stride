...
class DistributedBattleBaseAI(DistributedObjectAI.DistributedObjectAI, BattleBase):
    ...
    def __init__(self, air, zoneId, finishCallback = None, maxSuits = 4, bossBattle = 0, interactivePropTrackBonus = -1):
        ...
        self.toonMerits = {}
        self.toonParts = {}
        self.battleCalc = BattleCalculatorAI(self)
        if self.air.suitInvasionManager.getInvading() and self.air.newsManager.isHolidayRunning(ToontownGlobals.MORE_XP_HOLIDAY):
            mult = getMoreXpHolidayAndInvasionMultiplier()
            self.battleCalc.setSkillCreditMultiplier(mult)
        elif self.air.suitInvasionManager.getInvading(): 
            mult = getInvasionMultiplier()
            self.battleCalc.setSkillCreditMultiplier(mult)
        elif self.air.newsManager.isHolidayRunning(ToontownGlobals.MORE_XP_HOLIDAY):
    ...
    def getMovie(self):
        ...
        for sa in self.suitAttacks:
            ...
            p = p + [sa[SUIT_HP_COL], sa[SUIT_CRIT_COL], sa[TOON_DIED_COL], sa[SUIT_BEFORE_TOONS_COL], sa[SUIT_TAUNT_COL]]
