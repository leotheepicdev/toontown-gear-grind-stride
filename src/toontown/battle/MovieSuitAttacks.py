...
def __doDamage(toon, dmg, crit):
    if dmg > 0 and toon.hp != None:
        if crit:
            bonus = 2
        else:
            bonus = 0
        toon.takeDamage(dmg, bonus=bonus, crit=crit)
    return
...
def doSuitAttack(attack):
    ...
    elif name == DROP_ANVIL:
        suitTrack = doDropAnvil(attack)
...
def getToonTrack(attack, damageDelay = 1e-06, damageAnimNames = None, dodgeDelay = 0.0001, dodgeAnimNames = None, splicedDamageAnims = None, splicedDodgeAnims = None, target = None, showDamageExtraTime = 0.01, showMissedExtraTime = 0.5):
    ...
    if dmg > 0:
        animTrack.append(getToonTakeDamageTrack(toon, attack, dmg, damageDelay, damageAnimNames, splicedDamageAnims, showDamageExtraTime))
        return animTrack
    else:
...
def getToonTakeDamageTrack(toon, attack, dmg, delay, damageAnimNames = None, splicedDamageAnims = None, showDamageExtraTime = 0.01):
    toonTrack = Sequence()
    ...
    indicatorTrack = Sequence(Wait(delay + showDamageExtraTime), Func(__doDamage, toon, dmg, attack['crit']))
    toonTrack.append(Func(toon.loop, 'neutral'))
    if toon.getHp() <= 0:
...
def doSongAndDance(attack):
    ...
    if dmg > 0:
        toonTracks.append(Sequence(geom.colorScaleInterval(suitTrack.getDuration() - 1.8, (0, 1, 0, 1)), Sequence(Func(suit.updateHealthBar, 0), Func(suit.showHpText, dmg * 2)), 
                              geom.colorScaleInterval(suitTrack.getDuration() - 3.8, suit.getColorScale())))
...
def doReOrg(attack):
    ...
    damageDelay = 1.7
    attackDelay = 1.7
    sprayEffect = BattleParticles.createParticleEffect(file='reorgSpray') # TODO(?): Fix this so it loads an actual particle
    suitTrack = getSuitTrack(attack)
...
def doEvilEye(attack):
    ...
    suitName = suit.getStyleName()
    if suitName == 'cr':
        posPoints = [Point3(-0.46, 5.05, 5.28), VBase3(-155.0, -20.0, 0.0)]
    elif suitName == 'tf':
        posPoints = [Point3(-0.4, 4.15, 5.01), VBase3(-155.0, -20.0, 0.0)]
    elif suitName == 'le':
        posPoints = [Point3(-0.64, 4.65, 5.91), VBase3(-155.0, -20.0, 0.0)]
    elif suitName == 'rb':
        posPoints = [Point3(-0.64, 4.75, 5.91), VBase3(-155.0, -20.0, 0.0)]
    else:
        posPoints = [Point3(-0.4, 4.15, 5.01), VBase3(-155.0, -20.0, 0.0)]
    appearDelay = 0.8
    suitHoldStart = 1.06
    ...
    eyeAppearTrack = Sequence(Wait(suitHoldStart), Func(__showProp, eye, suit, posPoints[0], posPoints[1]), LerpScaleInterval(eye, suitHoldDuration, Point3(0.9, 0.9, 0.9)), Wait(eyeHoldDuration * 0.3), LerpHprInterval(eye, 0.02, Point3(205, 40, 0)), Wait(eyeHoldDuration * 0.7), Func(battle.movie.needRestoreRenderProp, eye), Func(eye.wrtReparentTo, battle))
...
def doCogUp(attack):
    ...
    tauntIndex = attack['taunt']
    taunt = getAttackTaunt(attack['name'], tauntIndex)
    suitTrack = Parallel(Sequence(Func(suit.setChatAbsolute, taunt, CFSpeech | CFTimeout), ActorInterval(suit, 'effort', startFrame=50), Func(suit.clearChat)))
    suitTracks = Parallel()
    for suit in suits:
        geom = suit.getGeomNode()
        suitTracks.append(Sequence(geom.colorScaleInterval(duration / 2, (0, 1, 0, 1)), Sequence(Func(suit.updateHealthBar, 0), Func(suit.showHpText, int(ceil(suit.getMaxHP() / 2)))), 
                          geom.colorScaleInterval(duration / 2, suit.getColorScale())))
    suitTrack.append(suitTracks)
    return suitTrack

def doDropAnvil(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    toon = target['toon']
    dmg = target['hp']
    dropDelay = 4
    anvil = globalPropPool.getProp('anvil')
    suitTrack = getSuitTrack(attack)
    dropSequence = Sequence()
    dropSequence.append(Wait(dropDelay))
    dropSequence.append(Func(anvil.reparentTo, toon))
    if dmg > 0:
        dropSequence.append(Func(anvil.setZ, toon.getHeight()))
    dropSequence.append(ActorInterval(anvil, 'anvil'))
    dropSequence.append(Func(anvil.reparentTo, hidden))
    toonTrack = getToonTrack(attack, 4.15, ['slip-backward'], 3.5, ['sidestep'])
    if dmg > 0:
        soundTrack = getSoundTrack('AA_drop_anvil.ogg', delay=4, node=toon)
    else:
        soundTrack = getSoundTrack('AA_drop_anvil_miss.ogg', delay=4, node=toon)
    return Parallel(suitTrack, toonTrack, dropSequence, soundTrack)
 
def __makeBrickWall(target, time = 1):
    wall = loader.loadModel('phase_3/models/misc/box')
    wall.reparentTo(target)
    wall.setPos(-1.45, 2, -2.75)
    wall.setColorScale(0.9, 0.9, 0.9, 1)
    wall.setTexture(loader.loadTexture('phase_9/maps/bricks.jpg'), 1)
    wall.hide()
    wallScale = (3, 0.3, 0.01)
    wall.setScale(*wallScale)
    sfx = loader.loadSfx('phase_9/audio/sfx/CHQ_VP_tractor_treads.ogg')
    growSequence = Parallel(SoundInterval(sfx), Func(wall.show), wall.scaleInterval(time, (wallScale[0], wallScale[1], target.getHeight() + 0.5), wallScale, blendType='easeInOut'))
...
def doBarrier(attack):
    ...
    suitTracks = Parallel()
    for suit in suits:
        wall, makeWallTrack = __makeBrickWall(suit, 2)
        shrinkWallTrack = __shrinkBrickWall(wall, 2)
        defDebuff, defenseMaxedOut = suit.getDefenseDebuff()
        if defenseMaxedOut:
            suitTracks.append(Sequence(Wait(2), makeWallTrack, Parallel(Sequence(Func(suit.updateHealthBar, 0), Func(suit.showHpText, int(ceil(suit.getMaxHP() / 2)))), Wait(1.5)), shrinkWallTrack))
        else:
            suitTracks.append(Sequence(Wait(2), makeWallTrack, Parallel(Func(MovieUtil.indicateDefenseRise, suit), Wait(1.5)), shrinkWallTrack))
    suitTrack.append(suitTracks)
    return suitTrack