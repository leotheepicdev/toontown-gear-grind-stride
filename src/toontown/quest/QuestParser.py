...
from toontown.base import TTLocalizer, ToontownBattleGlobals
...
class NPCMoviePlayer(DirectObject.DirectObject):
    ...
    def play(self):
        ...
        for line in lines:
            ...
            if self.isLocalToon:
                ...
                elif command == 'BLACK_TOON_LISTEN':
                    iList.append(self.parseBlackToonListen(line))
    ...
    def parseBlackToonListen(self, line):
        token, enable = line
        if enable:
            ...
            def enableBlackToonListen():
                self.acceptOnce(SpeedChatGlobals.SCStaticTextMsgEvent, phraseSaid)

            return Func(enableBlackToonListen)
        else:

            def disableBlackToonListen():
                self.ignore(SpeedChatGlobals.SCStaticTextMsgEvent)

            return Func(disableBlackToonListen)
