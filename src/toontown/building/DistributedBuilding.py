...
FO_DICT = {'s': 'tt_m_ara_cbe_fieldOfficeMoverShaker',
 ...
 'c': 'tt_m_ara_cbe_fieldOfficeBigCheese'}
...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def setupSuitBuilding(self, nodePath):
        ...
        if self.numFloors > 7:
            self.notify.warning('Floors are bigger than 7: %s' % self.numFloors)
        elif self.numFloors < 1:
            self.notify.warning('Floors are less than 1: %s' % self.numFloors)
        suitNP = dnaStore.findNode('suit_landmark_' + chr(self.track) + str(min(self.numFloors, 7)))
