...
from toontown.base.ToontownGlobals import ToonHall
...
class DistributedBuildingAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def suitTakeOver(self, suitTrack, suitLevel):
        # TODO: possibly add an option to force the difficulty when we find a use for it.
        if not self.isToonBlock():
            return
        self.updateSavedBy(None)
        difficulty = SuitBuildingGlobals.determineDifficulty(suitLevel)
        numFloors = SuitBuildingGlobals.determineFloors(difficulty)
    ...
    def cogdoTakeOver(self, suitTrack, suitLevel, forcedDifficulty=None):
        if not self.isToonBlock():
            return
        self.updateSavedBy(None)
        self.track = suitTrack
        self.realTrack = suitTrack
        if forcedDifficulty != None:
            # we have this so we can make the field office magic word work
            self.difficulty = forcedDifficulty
        else:
            self.difficulty = SuitBuildingGlobals.determineDifficulty(suitLevel)
        self.numFloors = 0
        self.becameSuitTime = time.time()
    ...
    def enterSuit(self):
        ...
        self.planner = CogBuildingPlannerAI.CogBuildingPlannerAI(self.numFloors, self.difficulty, self.track, interiorZoneId)
