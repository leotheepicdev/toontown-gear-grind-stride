...
class DistributedFishingSpotAI(DistributedObjectAI):
    ...
    def requestEnter(self):
        ...
        if av.occupyingFishingDock:
            self.air.writeServerEvent('suspicious', avId, 'Toon tried to enter more than one fishing spot')
            self.air.punishmentManager.kick(avId, 106, 'Tried to enter more than one fishing spot. Nice try!')
            return
        av.occupyingFishingDock = True
