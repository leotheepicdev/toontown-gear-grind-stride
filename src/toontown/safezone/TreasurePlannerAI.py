...
from DistributedBeanBagTreasureAI import DistributedBeanBagTreasureAI
from TreasureGlobals import AvailableBeanBagChoices
import random
...
class TreasurePlannerAI(DirectObject.DirectObject):
    ...
    def placeTreasure(self, index):
        ...
        if random.random() <= 0.1:
            treasure = DistributedBeanBagTreasureAI(simbase.air, self, spawnPoint[0], spawnPoint[1], spawnPoint[2])
            treasure.setValue(random.choice(AvailableBeanBagChoices))
