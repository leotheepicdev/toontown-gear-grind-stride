...
from toontown.coghq import CogHQBossPlannerAI
...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def invokeSuitPlanner(self, buildingCode, skelecog, skelecogRandom=0, virtualRandom=0):
        planner = CogHQBossPlannerAI.CogHQBossPlannerAI(1, buildingCode, self.dna.dept, self.zoneId)
        ...
        if skelecog:
            for suit in suits['activeSuits']:
                ...
                if virtualRandom:
                    if random.random() <= ToontownGlobals.VIRTUAL_BOSS_CHANCE:
                        suit.makeRandomVirtual()
    ...
    def zapToon(self, x, y, z, h, p, r, bpx, bpy, attackCode, timestamp):
        ...
        if toon:
            ...
            if self.dna.dept == 's':
                damage = ToontownGlobals.SellbotBossDamageLevels.get(attackCode)
            else:
    ...
    def setMostValuablePlayer(self, mvpId, mvpScore):
        self.mvpId = mvpId
        self.mvpScore = mvpScore

    def d_setMostValuablePlayer(self, mvpId, mvpScore):
        self.sendUpdate('setMostValuablePlayer', [mvpId, mvpScore])
		
    def b_setMostValuablePlayer(self, mvpId, mvpScore):
        self.setMostValuablePlayer(mvpId, mvpScore)
        self.d_setMostValuablePlayer(mvpId, mvpScore)

    def getMostValuablePlayer(self):
        return (self.mvpId, self.mvpScore)
    ...
    def increaseScore(self, amount):
        ...
        if avId in self.scores:
            if self.scores[avId] == 255:
                return
            self.scores[avId] += 1
        else:
            self.scores[avId] = 1
			
    def calculateAndRewardMVP(self):
        if not self.scores:
            return
        for avId in self.scores:
            av = self.air.doId2do.get(avId)
            if not av:
                del self.scores[avId]
        mvpId = max(self.scores, key=lambda x: self.scores.get(x))
        mvpAv = self.air.doId2do.get(mvpId)
        self.b_setMostValuablePlayer(mvpId, self.scores[mvpId])
        mvpAv.addMoney(self.scores[mvpId] * 2)
