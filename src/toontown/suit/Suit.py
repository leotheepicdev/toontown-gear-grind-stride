from panda3d.core import ColorBlendAttrib, CollideMask, GeomNode, NodePath, Point3, Texture, Vec4
...
bSize = 5.29
cSize = 4.14
SuitDialogTypes = ['grunt', 'murmur', 'statement', 'question', 'statement']
SkelSuitDialogArray = [loader.loadSfx('phase_5/audio/sfx/Skel_COG_VO_%s.ogg' % dialog) for dialog in SuitDialogTypes]
SuitDialogArray = [loader.loadSfx('phase_3.5/audio/dial/COG_VO_%s.ogg' % dialog) for dialog in SuitDialogTypes]
...
nd = (('pickpocket', 'pickpocket', 5),
 ...
 ('magic3', 'magic3', 5),
 ('smile', 'smile', 5))
gh = (('speak', 'speak', 5), ('pen-squirt', 'fountain-pen', 5), ('rubber-stamp', 'rubber-stamp', 5), ('effort', 'effort', 5))
ms = (('effort', 'effort', 5),
 ('throw-paper', 'throw-paper', 5),
 ('stomp', 'stomp', 5),
 ('quick-jump', 'jump', 6))
...
ls = (('throw-paper', 'throw-paper', 5), ('throw-object', 'throw-object', 5), ('hold-pencil', 'hold-pencil', 5), ('magic2', 'magic2', 5))
rb = (('glower', 'glower', 5), ('pickpocket', 'pickpocket', 5), ('cigar-smoke', 'cigar-smoke', 8), ('magic3', 'magic3', 5), ('golf-club-swing', 'golf-club-swing', 5))
...
def attachSuitHead(node, suitName):
    if suitName in SuitGlobals.unlistedSuits:
        suitIndex = 0 # Doesn't matter since we only need this for TownBattleCogPanel
    else:
...
class Suit(Avatar.Avatar):
    ...
    virtualColors = {
      ToontownGlobals.VIRTUAL_ATK: (1, 0, 0, 1), # Red
      ToontownGlobals.VIRTUAL_HP: (0, 1, 0, 1), # Green
      ToontownGlobals.VIRTUAL_DEF: (0, 0, 1, 1), # Blue
      ToontownGlobals.VIRTUAL_ACC: (1, 0, 1, 1), # Pink
      ToontownGlobals.VIRTUAL_LUCK: (0.9, 0.8, 0, 1) # Gold
    }
    ...
    def setSuitClothes(self, modelRoot = None):
        ...
        phase = 3.5
        torsoTexName = 'phase_%s/maps/%s_blazer.jpg' % (phase, dept)
        legTexName = 'phase_%s/maps/%s_leg.jpg' % (phase, dept)
        armTexName = 'phase_%s/maps/%s_sleeve.jpg' % (phase, dept)
        torsoTex = loader.loadTexture(torsoTexName)
        torsoTex.setMinfilter(Texture.FTLinearMipmapLinear)
        torsoTex.setMagfilter(Texture.FTLinear)
        legTex = loader.loadTexture(legTexName)
        legTex.setMinfilter(Texture.FTLinearMipmapLinear)
        legTex.setMagfilter(Texture.FTLinear)
        armTex = loader.loadTexture(armTexName)
    ...
    def makeExecutive(self, modelRoot=None):
        ...
        self.isExecutive = 1
        dept = self.style.dept
        torsoTex = loader.loadTexture('phase_3.5/maps/%s_blazer_executive.jpg' % dept)
        torsoTex.setMinfilter(Texture.FTLinearMipmapLinear)
        torsoTex.setMagfilter(Texture.FTLinear)
        legTex = loader.loadTexture('phase_3.5/maps/%s_leg_executive.jpg' % dept)
        legTex.setMinfilter(Texture.FTLinearMipmapLinear)
        legTex.setMagfilter(Texture.FTLinear)
        armTex = loader.loadTexture('phase_3.5/maps/%s_sleeve_executive.jpg' % dept)
        armTex.setMinfilter(Texture.FTLinearMipmapLinear)
        armTex.setMagfilter(Texture.FTLinear)
        modelRoot.find('**/torso').setTexture(torsoTex, 1)
        modelRoot.find('**/arms').setTexture(armTex, 1)
        modelRoot.find('**/legs').setTexture(legTex, 1)
        modelRoot.find('**/hands').setColorScale(SuitGlobals.ExecutiveHandColor)
    ...
    def getLoseActor(self):
        if self.loseActor == None:
            if not self.isSkeleton:
                ...
                if self.virtual > 0:
                    self.makeVirtual(self.virtual, self.loseActor)
    ...
    def makeVirtual(self, virtual, modelRoot=None):
        if not modelRoot:
            modelRoot = self
        parts = modelRoot.findAllMatches('*')
        for thingIndex in xrange(0, parts.getNumPaths()):
            ...
            if thing.getName() not in ('joint_attachMeter', 'joint_nameTag', 'def_nameTag', 'nametag3d'):
                thing.setColorScale(*self.virtualColors[virtual])
