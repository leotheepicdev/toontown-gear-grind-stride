...
class DistributedSuitBase(DistributedAvatar.DistributedAvatar, Suit.Suit, SuitBase.SuitBase):
    ...
    def setVirtual(self, virtual):
        ...
        if self.getVirtual() > 0:
            ...
            if self.getVirtual() == ToontownGlobals.VIRTUAL_HP:
                self.maxHP *= ToontownGlobals.VIRTUAL_HP_MULT
                self.currHP *= ToontownGlobals.VIRTUAL_HP_MULT
            name = TTLocalizer.SuitBaseNameVirtual % TTLocalizer.virtualType2Name[virtual]
            nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': name,
             'dept': self.getStyleDept(),
             'level': '%s' % self.getActualLevel()}
            self.setName(name)
    ...
    def setWaiter(self, flag):
        ...
        if flag:
            ...
            if self.getSkeleRevives() > 0:
                nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': self.name,
                 'dept': self.getStyleDept(),
                 'level': '%s%s%s' % (self.getActualLevel(), TTLocalizer.WaiterPostFix, TTLocalizer.SkeleRevivePostFix % (self.skeleRevives + 1))}
                self.setDisplayName(nameInfo)
            else:
                nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': self.name,
                 'dept': self.getStyleDept(),
                 'level': '%s%s' % (self.getActualLevel(), TTLocalizer.WaiterPostFix)}
                self.setDisplayName(nameInfo)
    ...
    def setExecutive(self, flag):
        ...
        if flag:
            ...
            if self.getSkeleRevives() > 0:
                nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': self.name,
                 'dept': self.getStyleDept(),
                 'level': '%s%s%s' % (self.getActualLevel(), TTLocalizer.SkeleRevivePostFix % (self.skeleRevives + 1), TTLocalizer.ExecutivePostFix)}
                self.setDisplayName(nameInfo)
            else:
                nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': self.name,
                 'dept': self.getStyleDept(),
                 'level': '%s%s' % (self.getActualLevel(), TTLocalizer.ExecutivePostFix)}
                self.setDisplayName(nameInfo)
