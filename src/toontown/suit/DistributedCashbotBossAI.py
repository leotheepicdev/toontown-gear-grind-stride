...
from toontown.goon import DistributedCashbotBossGoonAI
...
class DistributedCashbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def makeGoon(self, side = None):
        ...
        if self.getBattleThreeTime() > 1.0:
            goon.STUN_TIME = 4
            goon.b_setupGoon(velocity=8, hFov=90, attackRadius=20, strength=45, scale=1.8)
        else:
            goon.STUN_TIME = self.progressValue(30, 8)
            goon.b_setupGoon(velocity=self.progressRandomValue(3, 7), hFov=self.progressRandomValue(70, 80), attackRadius=self.progressRandomValue(6, 15), strength=int(self.progressRandomValue(5, 45)), scale=self.progressRandomValue(0.5, 1.5))
        goon.request(side)
        return
    ...
    def __doneVictory(self, avIds):
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                if toon.isCogSuitMaxed(2):
                    toon.addResistanceMessage(self.rewardId, 2)
                else:
