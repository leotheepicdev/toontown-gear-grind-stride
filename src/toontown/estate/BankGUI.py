...
class BankGui(DirectFrame):
    ...
    def __init__(self, doneEvent, allowWithdraw = 1):
        ...
        guiButtonImg = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR'))
        ...
        self.withdrawAll = DirectButton(parent=self, relief=None, pos=(0, 0, 0.35), image=guiButtonImg, scale=1.0, text=TTLocalizer.BankGuiWithdrawAll, text_scale=0.06, text_pos=(0, -0.02), command=self.__withdrawAll)        
    ...
    def __updateTransaction(self, amount):
        ...
        if newBankMoney <= 0 or newJarMoney >= maxJarMoney:
            ...
            self.withdrawAll['state'] = DGG.DISABLED
            hitLimit = 1
        else:
            self.withdrawArrow['state'] = DGG.NORMAL
            self.withdrawAll['state'] = DGG.NORMAL
    ...
    def __withdrawAll(self):
        messenger.send('wakeup')
        hitLimit, jar, bank, trans = self.__updateTransaction(-base.localAvatar.getBankMoney())
