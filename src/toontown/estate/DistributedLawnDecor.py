from panda3d.core import BitMask32, CollideMask, CollisionHandler, CollisionHandlerQueue, CollisionNode, CollisionRay, CollisionSphere, CollisionTraverser, CollisionTube, NodePath, Point3, Vec3, lookAt
...
class DistributedLawnDecor(DistributedNode.DistributedNode, NodePath, ShadowCaster.ShadowCaster):
    ...
    def generateDigupTrack(self, toon):
        ...
        pos.setZ(pos[2] - 1)
        track = Parallel()
        track.append(Sequence(ActorInterval(toon, 'start-dig'), Parallel(ActorInterval(toon, 'loop-dig', loop=1, playRate=1.2, duration=2.77), Sequence(Wait(0.25), SoundInterval(sound, node=toon, duration=0.55), Wait(0.8), SoundInterval(sound, node=toon, duration=0.55))), ActorInterval(toon, 'start-dig', playRate=-1), Func(toon.loop, 'neutral'), Func(toon.detachShovel)))
        return track
    ...
    def doFinishPlantingTrack(self, avId):
        ...
        if self.model:
            ...
            self.movie.append(LerpFunc(self.model.setAlphaScale, fromData=0, toData=1, duration=1))
