from panda3d.core import Camera, Fog, Vec4
...
from toontown.distributed.TelemetryLimiter import RotationLimitToH, TLGatherAllAvs, TLNull
...
class Estate(Place.Place):
    ...
    def __init__(self, loader, avId, zoneId, parentFSMState, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM('Estate', [State.State('init', self.enterInit, self.exitInit, ['final',
         ...
         State.State('pet', self.enterPet, self.exitPet, ['walk', 'teleportOut']),
         State.State('purchase', self.enterPurchase, self.exitPurchase, ['walk'])], 'init', 'final')
