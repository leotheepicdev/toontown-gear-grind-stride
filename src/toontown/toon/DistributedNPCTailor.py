    ...
    def setMovie(self, mode, npcId, avId, timestamp):
        ...
        if mode == NPCToons.PURCHASE_MOVIE_TIMEOUT:
            ...
            if self.isLocalToon:
                ...
                self.cameraWork = camera.posHprInterval(2, Point3(-5, 9, self.getHeight() - 0.04), Point3(-150, -2, 0), blendType='easeOut')
                self.cameraWork.start()
            if self.browsing == 0:
                if self.roomAvailable == 0:
                    self.setChatAbsolute(TTLocalizer.STOREOWNER_NOROOM, CFSpeech | CFTimeout)
                else:
                    self.setChatAbsolute(TTLocalizer.STOREOWNER_GREETING1, CFSpeech | CFTimeout)
