...
class InventoryNew(InventoryBase.InventoryBase, DirectFrame):
    ...
    def load(self):
        ...
        for track in xrange(0, len(Tracks)):
            ...
            adjustLeft = -0.065
            self.trackNameLabels.append(DirectLabel(text=TextEncoder.upper(Tracks[track]), parent=self.trackRows[track], pos=(-0.72 + adjustLeft, -0.1, 0.01), scale=TTLocalizer.INtrackNameLabels, relief=None, text_fg=(0.85, 0.85, 0.85, 1), text_shadow=(0, 0, 0, 1), text_font=getMinnieFont(), text_align=TextNode.ALeft, textMayChange=0))
    ...
    def showDetail(self, track, level, event = None):
        ...
        if self.propAndOrganicBonusStack:
            ...
            if damageBonus:
                damageBonusStr = TTLocalizer.InventoryDamageBonus % damageBonus
        propAcc = AvPropAccuracy[track][level]
        if (organicBonus or propBonus) and track == LURE_TRACK:
            propAcc = AvLureBonusAccuracy[level]
        accString = '%s%%' % propAcc
