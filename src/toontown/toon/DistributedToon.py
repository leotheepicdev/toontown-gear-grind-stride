from panda3d.core import CollideMask, NodePath, Point3, TextNode, VBase4, Vec4
import copy
...
from lib.nametag import NametagGroup
...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def __init__(self, cr, bFake = False):
        ...
        self.activeCheesyEffect = ToontownGlobals.CENormal
    ...
    def requestCheesyEffect(self, cheesyEffect):
        if cheesyEffect not in self.cheesyEffects:
            return
        self.sendUpdate('requestCheesyEffect', [cheesyEffect])
    ...
    def giftJellybeans(self, toId, amount):
        self.sendUpdate('giftJellybeans', [toId, amount])
        
    def giftJellybeansResult(self, callback):
        messenger.send('giftJellybeansResult', [callback])
    ...
    def setNametagColor(self, nametagColor):
        ...
        self.nametag.setColorCode(nametagColor)
        self.setNametagName()
        if self == base.localAvatar:
            messenger.send('refreshNametagColor')
    ...
    def requestNametagColor(self, nametagColor):
        ...
        self.sendUpdate('requestNametagColor', [nametagColor])
        
    def getNametagPanel(self):
        if hasattr(self, 'nametagPanel'):
            return self.nametagPanel
        return 0
        
    def setNametagPanel(self, nametagPanel):
        self.nametagPanel = nametagPanel
        self.nametag.setPanelType(nametagPanel)
        
    def requestNametagPanel(self, nametagPanel):
        self.sendUpdate('requestNametagPanel', [nametagPanel])
		
    def setPlayerType(self, playerType):
        self.playerType = playerType
        self.nametag.playerType = playerType
        if playerType == NametagGroup.CCNonPlayer:
            # since npcs cannot call setNametagColor, let's just set their nametag color here. honestly shouldn't call setColorCode twice for normal toons.
            self.nametag.setColorCode(playerType)
            self.setNametagName()
		
    def considerUnderstandable(self):
        if base.localAvatar.isTrueFriends(self.doId):
            self.understandable = 2
        elif self.playerType == NametagGroup.CCNonPlayer:
    ...
    def showHpText(self, number, bonus = 0, scale = 1, crit = 0, hasInteractivePropBonus = False):
        if self.HpTextEnabled and not self.ghostMode:
            if number != 0:
                ...
                if number < 0:
                    if crit:
                        self.HpTextGenerator.setText('%s \n%s' % (str(number), TTLocalizer.CriticalStrike))
                    else:
    ...
    def warnToon(self, reason):
        self.setSystemMessage(base.localAvatar.doId, TTLocalizer.WarnMessage % reason, WTSystem)
    ...
    def setTPose(self):
        ...
        if self.isDisguised:
            ...
            if self.suit.isRental:
                ...
                rental = False
            self.putOnSuit(suitType, True, rental, True)
    ...
    def updateTrolleyPosition(self, trolleyPosition):
        self.sendUpdate('updateTrolleyPosition', [trolleyPosition])
...
def blackToon():
    """
    Ask the black toon manager to turn you into a black toon.
    """
    base.cr.blackToonMgr.requestBlackToonTransformation()
...
def toggleGM():
    ...
    if invoker.gmIcon:
        ...
        invoker.setGMIcon(invoker.sessionGMIcon)
		
@magicWord(category=CATEGORY_ADMINISTRATOR, types=[int])
def gmIcon(accessLevel):
    invoker = spellbook.getInvoker()
    if accessLevel not in (100, 200, 300, 400, 500, 600, 700):
        return 'Invalid access level specified'
    invoker.removeGMIcon()
    invoker.setGMIcon(accessLevel)
    return 'Changed GM icon to %s for this session.' % accessLevel
