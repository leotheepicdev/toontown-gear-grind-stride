...
def doHappyDance(toon, volume = 1):
    sfx = loader.loadSfx('phase_5/audio/sfx/AA_heal_happydance.ogg')
    track = Sequence(Func(toon.play, 'happy-dance'), Func(base.playSfx, sfx, volume=volume, node=toon))
...
def doHypnotize(toon, volume=1):
    sfx = loader.loadSfx('phase_5/audio/sfx/TL_hypnotize.ogg')
    track = Sequence(Func(toon.play, 'hypnotize'), Wait(0.5), Func(base.playSfx, sfx, volume=volume, node=toon))
...
def doTaunt(toon, volume=1):
    ...
    track = Sequence(Func(toon.blinkEyes), Func(toon.angryEyes), Func(toon.play, 'taunt'), Func(base.playSfx, sfx, volume=volume, node=toon))
    exitTrack = Sequence(Func(toon.blinkEyes), Func(toon.normalEyes))
    duration = toon.getDuration('taunt')
    return (track, duration, exitTrack)
    
def doTPose(toon, volume=1):
    track = Sequence(Func(toon.setTPose))
    return (track, 0, None)
...
class TTEmote(Emote.Emote):
    ...
    def __init__(self):
        ...
        self.headEmotes = [2,
         17,
         18]
        if len(self.emoteFunc) != len(TTLocalizer.EmoteList):
            self.notify.error('Emote.EmoteFunc and TTLocalizer.EmoteList are different lengths.')
        self.track = None
    ...
    def disable(self, index, toon):
        if isinstance(index, types.StringType):
            index = TTLocalizer.EmoteFuncDict[index]
