from panda3d.core import BitMask32, CollideMask, CollisionNode, CollisionSphere, NodePath, Notify
...
from toontown.avatar import DistributedAvatarAI, DistributedPlayerAI
from otp.otpbase import OTPGlobals
from toontown.battle import SuitBattleGlobals
from toontown.catalog import CatalogAccessoryItem, CatalogBeanItem, CatalogItem, CatalogItemList
...
from toontown.base import TTLocalizer, ToontownBattleGlobals, ToontownGlobals
...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def __init__(self, air):
        ...
        self.tfRequest = (0, 0)
        self.toonTag = ''
        self.occupyingFishingDock = False
        self.avBeingEjected = False
    ...
    def setLocation(self, parentId, zoneId):
        ...
        if not isinstance(self, DistributedNPCToonBaseAI):
            if 100 <= zoneId < ToontownGlobals.DynamicZonesBegin:
                ...
                if hood not in self.getHoodsDiscovered():
                    self.discoveredHood(hood)
                self.b_setLastHood(hood)
                self.b_setDefaultZone(hood)

            if self.getToonTarget().getMaxHp() < ToontownGlobals.MaxHpLimit:
                self.maxAlphaToon()
    ...
    def getToonTarget(self):
        invokerId = self.air.getAvatarIdFromSender()
        target = self.air.doId2do.get(invokerId)
        return target

    def maxAlphaToon(self):
        # Maxes every toon's stats excluding tasks. 
        # This should only be for alpha testers. Remove this after alpha is over.
        target = self.getToonTarget()
        ...
        # Discover All Hoods:
        target.b_setHoodsDiscovered(ToontownGlobals.HoodsDiscoveredMax)

        # Unlock all of the emotes:
        emotes = list(target.getEmoteAccess())
        for emoteId in TTLocalizer.EmoteFuncDict.values():
    ...
    def ban(self, comment):
        simbase.air.punishmentManager.ban(self.doId, -1, comment, 0)
    ...
    def canWearSuit(self, avatarId, zoneId):
    ...
    def handleLogicalZoneChange(self, newZoneId, oldZoneId):
        ...
        if not simbase.air.cogSuitMessageSent:
            if self.cogIndex != -1 and not self.canWearSuit(self.doId, newZoneId):
                ...
                if simbase.config.GetBool('want-ban-wrong-suit-place', False):
                    ...
                    dislId = self.DISLid
                    simbase.air.punishmentManager.ban(self.doId, -1, commentStr, 0)
    ...
    def attemptAddNPCFriend(self, npcFriend, method=Quests.WithCheat, bonusCalls=0):
        numCalls = simbase.air.config.GetInt('sos-card-reward', 1) + bonusCalls
        if numCalls <= 0:
    ...
    def correctToon(self):
        if self.getMaxFishTank() != FishGlobals.MaxTank:
            self.b_setMaxFishTank(FishGlobals.MaxTank)
        if self.getMaxMoney() != ToontownGlobals.MaxMoney:
            self.b_setMaxMoney(ToontownGlobals.MaxMoney)
        if self.getMaxBankMoney() != ToontownGlobals.MaxBankMoney:
            self.b_setMaxBankMoney(ToontownGlobals.MaxBankMoney)
    ...
    def setAnimState(self, animName, animMultiplier, timestamp = 0):
        if animName not in ToontownGlobals.ToonAnimStates:
            ...
            if config.GetBool('want-ban-animstate', 1):
                simbase.air.punishmentManager.ban(self.doId, -1, desc, 0)
            else:
    ...
    def doQuestMerits(self, dept):
        if not CogDisguiseGlobals.isSuitComplete(self.cogParts, dept) or self.isCogSuitMaxed(dept):
            return
        totalMerits = CogDisguiseGlobals.getTotalMerits(self, dept)
        merits = self.cogMerits[dept]
        if totalMerits == 0 or merits >= totalMerits:
            return
        self.cogMerits[dept] = min(totalMerits, int(merits + (totalMerits / 5)))
    ...
    def isCogSuitMaxed(self, dept):
        return self.cogLevels[dept] == ToontownGlobals.MaxCogSuitLevel
    ...
    def b_setCheesyEffects(self, cheesyEffects):
        self.setCheesyEffects(cheesyEffects)
        self.d_setCheesyEffects(cheesyEffects)

    def setCheesyEffects(self, cheesyEffects):
        self.cheesyEffects = cheesyEffects

    def d_setCheesyEffects(self, cheesyEffects):
        self.sendUpdate('setCheesyEffects', [cheesyEffects])

    def getCheesyEffects(self):
        return self.cheesyEffects

    def hasCheesyEffect(self, cheesyEffect):
        return cheesyEffect in self.cheesyEffects

    def requestCheesyEffect(self, cheesyEffect):
        if cheesyEffect not in self.cheesyEffects:
            return
        self.b_setCheesyEffect(cheesyEffect)

    def addCheesyEffect(self, cheesyEffect):
        if cheesyEffect in self.cheesyEffects:
            return
        self.cheesyEffects.append(cheesyEffect)
        self.b_setCheesyEffects(self.cheesyEffects)
        self.b_setCheesyEffect(cheesyEffect)

    def b_setCheesyEffect(self, cheesyEffect):
        self.setCheesyEffect(cheesyEffect)
        self.d_setCheesyEffect(cheesyEffect)

    def d_setCheesyEffect(self, cheesyEffect):
        self.sendUpdate('setCheesyEffect', [cheesyEffect])
    ...
    def b_setEmoteAccess(self, emoteAccess):
        self.setEmoteAccess(emoteAccess)
        self.d_setEmoteAccess(emoteAccess)

    def d_setEmoteAccess(self, emoteAccess):
        self.sendUpdate('setEmoteAccess', [emoteAccess])
    ...
    def addResistanceMessage(self, textId, amount=1):
    ...
    def giftJellybeans(self, toId, amount):
        if amount > self.getTotalMoney():
            return
        if amount < 0:
            return
        def handleQueryAvatar(dclass, fields, toId=toId, amount=amount):
            if dclass != self.air.dclassesByName['DistributedToonAI']:
                return
            deliverySchedule = fields['setDeliverySchedule'][0]
            giftSchedule = fields['setGiftSchedule'][0]
            mailboxContents = fields['setMailboxContents'][0]
            jbItemList = CatalogItemList.CatalogItemList(giftSchedule, store=CatalogItem.Customization | CatalogItem.DeliveryDate)
            if len(mailboxContents) + len(deliverySchedule) + len(giftSchedule) + len(jbItemList) >= ToontownGlobals.MaxMailboxContents:
                self.sendUpdate('giftJellybeansResult', [1])
                return
            jbItem = CatalogBeanItem.CatalogBeanItem(amount, 1)
            jbItem.giftTag = self.doId
            jbItem.deliveryDate = int(time.time() / 60 + .5)
            jbItemList.append(jbItem)
            jbItemListBlob = jbItemList.getBlob(store=CatalogItem.Customization | CatalogItem.DeliveryDate)
            self.takeMoney(amount)
            self.sendUpdate('giftJellybeansResult', [0])
            simbase.air.send(simbase.air.dclassesByName['DistributedToonAI'].aiFormatUpdate('setGiftSchedule', toId, toId, simbase.air.ourChannel, [jbItemListBlob]))
            
        self.air.dbInterface.queryObject(self.air.dbId, toId, handleQueryAvatar)
    ...
    if simbase.wantPets:
        ...
        def setSC(self, msgId):
            ...
            if msgId in [21006]:
                self.setHatePets(1)
            elif msgId in [21000, 21001, 21003, 21004, 21200, 
    ...
    def doBuildingTakeover(self, suitIndex):
        ...
        building.suitTakeOver(track, level)
    ...
    def doToonBuildingTakeover(self):
        ...
        self.notify.warning('toonTakeOverFromStreet %s %d' % (building.block, self.zoneId))
    ...
    def doCogdoTakeOver(self, suitIndex):
        ...
        building.cogdoTakeOver(track, level)
        return ['success', level, building.doId]

    def doCogInvasion(self, suitDeptIndex, suitTypeIndex, flags=0, summoned=True):
        if self.air.suitInvasionManager.getInvading():
            return ['busy', 0, 0]
        suitName = SuitDNA.getSuitName(suitDeptIndex, suitTypeIndex)
        suitIndex = SuitDNA.suitHeadTypes.index(suitName)
        if not self.air.suitInvasionManager.areSummonsAllowed():
            return ['fail', suitIndex, 0]
        if self.air.suitInvasionManager.beginInvasion(
                suitDeptIndex=suitDeptIndex, suitTypeIndex=suitTypeIndex, flags=flags, summoned=summoned):
            return ['success', suitIndex, 0]
        return ['fail', suitIndex, 0]
    ...
    def b_setNametagColors(self, nametagColors):
        self.d_setNametagColors(nametagColors)
        self.setNametagColors(nametagColors)

    def d_setNametagColors(self, nametagColors):
        self.sendUpdate('setNametagColors', [nametagColors])

    def setNametagColors(self, nametagColors):
        self.nametagColors = nametagColors
    
    def addNametagColor(self, nametagColor):
        if nametagColor in self.nametagColors:
            return

        self.nametagColors.append(nametagColor)
        self.b_setNametagColors(self.nametagColors)

    def getNametagColors(self):
        return self.nametagColors
    
    def requestNametagColor(self, nametagColor):
        if nametagColor not in self.nametagColors:
            return

        self.b_setNametagColor(nametagColor)
        
    def b_setNametagPanel(self, nametagPanel):
        self.d_setNametagPanel(nametagPanel)
        self.setNametagPanel(nametagPanel)

    def d_setNametagPanel(self, nametagPanel):
        self.sendUpdate('setNametagPanel', [nametagPanel])

    def setNametagPanel(self, nametagPanel):
        self.nametagPanel = nametagPanel

    def getNametagPanel(self):
        return self.nametagPanel
        
    def requestNametagPanel(self, nametagPanel):
        self.b_setNametagPanel(nametagPanel)
    ...
    def setTrolleyPosition(self, trolleyPosition):
        self.trolleyPosition = trolleyPosition

    def d_setTrolleyPosition(self, trolleyPosition):
        self.sendUpdate('setTrolleyPosition', [trolleyPosition])

    def b_setTrolleyPosition(self, trolleyPosition):
        self.setTrolleyPosition(trolleyPosition)
        self.d_setTrolleyPosition(trolleyPosition)

    def getTrolleyPosition(self):
        return self.trolleyPosition
        
    def updateTrolleyPosition(self, trolleyPosition):
        self.b_setTrolleyPosition(trolleyPosition)
        
    def setHoodsDiscovered(self, hoodsDiscovered):
        self.hoodsDiscovered = hoodsDiscovered
    
    def d_setHoodsDiscovered(self, hoodsDiscovered):
        self.sendUpdate('setHoodsDiscovered', [hoodsDiscovered])
        
    def b_setHoodsDiscovered(self, hoodsDiscovered):
        self.setHoodsDiscovered(hoodsDiscovered)
        self.d_setHoodsDiscovered(hoodsDiscovered)
        
    def getHoodsDiscovered(self):
        return self.hoodsDiscovered
        
    def discoveredHood(self, hood):
        self.hoodsDiscovered.append(hood)
        self.b_setHoodsDiscovered(self.hoodsDiscovered)

@magicWord(category=CATEGORY_PROGRAMMER, types=[str])
def cheesyEffect(value):
    """
    Modify the invoker's cheesy effect.
    """
    try:
        value = int(value)
    except:
        value = value.lower()
        if value not in ToontownGlobals.CEName2Id:
            return 'Invalid cheesy effect value: %s' % value
        value = ToontownGlobals.CEName2Id[value]
    if value < 0 or value > 15:
        return 'Invalid effect value: %s' % value
...
def unlocks():
    """
    Unlocks the target's emotions, and pet trick phrases.
    """
    target = spellbook.getTarget()
...
def nametagStyle(nametagStyle):
    if nametagStyle not in TTLocalizer.NametagFontNames:
        return 'Invalid nametag style.'
    nametagStyle = TTLocalizer.NametagFontNames.index(nametagStyle)
    target = spellbook.getTarget()
...
@magicWord(category=CATEGORY_MODERATOR, types=[str])
def nametagColor(nametagColor):
    if nametagColor not in TTLocalizer.NametagColorNames:
        return 'Invalid nametag color.'
    nametagColor = TTLocalizer.NametagColorNames.index(nametagColor)
    target = spellbook.getTarget()
    target.b_setNametagColor(nametagColor)
    target.addNametagColor(nametagColor)
    return 'Nametag color set to: %s.' % TTLocalizer.NametagColorNames[nametagColor]
...
def summoncogdo(track="s", forcedDifficulty=5):
    ...
    building.cogdoTakeOver(track, 0, forcedDifficulty=forcedDifficulty)
    return 'Successfully spawned cogdo with track %s and difficulty %d' % (track, forcedDifficulty)
...
def endInvasion():
    if not simbase.air.suitInvasionManager.getInvading():
        return 'This district is currently not invading.'
    simbase.air.suitInvasionManager.endInvasion()
    return 'Ending invasion.'
    
@magicWord(category=CATEGORY_MODERATOR)
def discoverHoods():
    av = spellbook.getInvoker()
    av.b_setHoodsDiscovered(ToontownGlobals.HoodsDiscoveredMax)
    return 'Discovered all hoods.'
	
@magicWord(category=CATEGORY_PROGRAMMER)
def maxCarry(value=100):
    if value < 0:
        return "Can't make max carry less than 0!"
    av = spellbook.getInvoker()
    av.b_setMaxCarry(value)
    return 'Set max carry to %s!' % value
    
@magicWord(category=CATEGORY_MODERATOR)
def toggleKart():
    """
    Allows us to toggle our kart.
    """
    av = spellbook.getInvoker()
    av.toggleKart()
