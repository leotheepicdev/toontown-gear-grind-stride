...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def putOnSuit(self, suitType, setDisplayName = True, rental = False, tPose=False):
        if self.isDisguised:
            self.takeOffSuit()
        ...
        if self.isLocal():
            ...
            ToontownGlobals.ToonRotateSpeed = ToontownGlobals.SuitRotateSpeed
            if self.hasTrackAnimToSpeed():
    ...
    def setGMIcon(self, access):
        ...
        if access == 300:
            icon = loader.loadModel('phase_3.5/models/gui/tt_m_gui_gm_toonResistance_fist').find('**/*fistIcon*')

        elif access in [500, 600, 700]:
            icon = loader.loadModel('phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle').find('**/*whistleIcon*')

        elif access in [200, 400]:
            icon = loader.loadModel('phase_3.5/models/gui/tt_m_gui_gm_toontroop_getConnected').find('**/*whistleIcon*')

        self.gmIcon = icon
        self.sessionGMIcon = access
    ...
    def setHeadPositions(self):
        ...
        if self.gmIcon:
            ...
            position += (2.3 if self.trophyStar else 2.6)
        ...
        if self.partyHat:
            if not self.headMeter and self.trophyStar:
                position += 0.2
