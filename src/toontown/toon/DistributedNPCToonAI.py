...
class DistributedNPCToonAI(DistributedNPCToonBaseAI):
    ...
    def chooseTrack(self, trackId):
        ...
        if trackId == -1:
            ...
            return
        if trackId in self.pendingTracks:
    ...
    def presentTrackChoice(self, av, avId, questId):
        self.busy = avId
        self.pendingAvId = avId
        self.pendingTracks = []
        ...
        for i, track in enumerate(av.getTrackAccess()):
            if track == 0:
                self.pendingTracks.append(i)
        self.sendUpdate('setMovie', [NPCToons.QUEST_MOVIE_TRACK_CHOICE, self.npcId, avId, self.pendingTracks, ClockDelta.globalClockDelta.getRealNetworkTime()])
        if not self.tutorial:
            self.clearTasks()
            self.task = self.uniqueName('clearMovie')
            taskMgr.doMethodLater(20.0, self.sendTimeoutMovie, self.task)
