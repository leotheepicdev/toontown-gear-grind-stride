...
class NametagColorShopGui:

    def __init__(self):
        self.index = 0
        self.id = time.time()
        self.lastNametagColor = base.localAvatar.nametagColor
        self.setupButtons()
        self.bindButtons()
    ...
    def setupButtons(self):
        ...
        self.title = DirectLabel(aspect2d, relief=None, text=TTLocalizer.NametagColorGuiTitle,
    ...
    def destroy(self):
        ...
        taskMgr.remove('runNametagColorCounter-%s' % self.id)
        
    def setClientNametagColor(self, nametagColor):
        base.localAvatar.nametagColor = nametagColor
        base.localAvatar.nametag.setColorCode(nametagColor)
        base.localAvatar.setNametagName()

    def __exit(self, state):
        self.destroy()
        self.setClientNametagColor(self.lastNametagColor)
        messenger.send('nametagColorShopDone', [state, self.index if state == NametagNPCGlobals.CHANGE else 0])
    ...
    def __updateIndex(self, offset):
        ...
        if (self.index + 1) >= len(TTLocalizer.NametagColorNames):
            self.upArrow['state'] = DGG.DISABLED
            hitLimit = 1
        else:
            self.upArrow['state'] = DGG.NORMAL
        
        if self.index in base.localAvatar.nametagColors:
            self.buyButton['state'] = DGG.DISABLED
            self.notice['text'] = TTLocalizer.NametagColorGuiSameColor
        else:
            self.buyButton['state'] = DGG.NORMAL
            self.notice['text'] = TTLocalizer.NametagGuiNotice % ToontownGlobals.NametagColorCost

        self.color['text'] = TTLocalizer.NametagColorNames[self.index]
        self.color['text_fg'] = NametagConstants.TOON_COLORS[self.index][0][0]
        self.setClientNametagColor(self.index)
        return hitLimit
    ...
    def __taskUpdate(self, delta, event):
        ...
        if not hitLimit:
            taskMgr.add(task, 'runNametagColorCounter-%s' % self.id)
