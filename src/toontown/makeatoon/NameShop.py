...
class NameShop(StateData.StateData):
    ...
    def load(self):
        ...
        self.randomButton = DirectButton(parent=aspect2d, relief=None, image=(self.squareUp,
         self.squareDown,
         self.squareHover,
         self.squareUp), clickSound=loader.loadSfx('phase_4/audio/sfx/tt_s_prp_sillyMeterArrow.ogg'), image_scale=(1.15, 1.1, 1.1), scale=(1.05, 1, 1), pos=(0, 0, -0.25), text=TTLocalizer.ShuffleButton, text_scale=0.06, text_pos=(0, -0.02), command=self.__randomName)
    ...
    def serverCreateAvatar(self):
        ...
        if not self.avExists or self.avExists and self.avId == 'deleteMe':
            base.cr.csm.sendCreateAvatar(style, '', self.index, self.toon.track1Type, self.toon.track2Type)
