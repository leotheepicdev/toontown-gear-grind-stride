...
class ShuffleButton:
    ...
    def __init__(self, parent, fetchEvent, shufflePos=(0, 0, -1)):
        self.parent = parent
    ...
    def load(self):
        ...
        gui.removeNode()
        del gui
        self.parentFrame = DirectFrame(parent=self.parent.parentFrame, relief=DGG.RAISED, pos=self.shufflePos, frameColor=(1, 0, 0, 0))
        ...
        self.shuffleBtn = DirectButton(parent=self.parentFrame, relief=None, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=halfButtonInvertScale, image1_scale=(-0.63, 0.6, 0.6), image2_scale=(-0.63, 0.6, 0.6), text=(TTLocalizer.ShuffleButton,
         ...
         TTLocalizer.ShuffleButton,
         ''), clickSound=loader.loadSfx('phase_4/audio/sfx/tt_s_prp_sillyMeterArrow.ogg'), text_font=ToontownGlobals.getInterfaceFont(), text_scale=TTLocalizer.SBshuffleBtn, text_pos=(0, -0.02), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), command=self.chooseRandom)
        ...
        self.decBtn = DirectButton(parent=self.parentFrame, relief=None, image=(shuffleArrowUp,
         shuffleArrowDown,
         shuffleArrowUp,
         shuffleArrowDisabled), clickSound=loader.loadSfx('phase_3/audio/sfx/GUI_create_toon_back.ogg'), image_scale=halfButtonScale, image1_scale=halfButtonHoverScale, image2_scale=halfButtonHoverScale, pos=(-0.202, 0, 0), command=self.__goBackHistory)
        self.decBtn.hide()
