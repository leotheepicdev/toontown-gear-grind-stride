from panda3d.core import CollideMask, CollisionHandler, CollisionHandlerQueue, CollisionNode, CollisionRay, CollisionTraverser, GeomNode, Lens, ModelPool, NodePath, Point3, SequenceNode, TextNode, Texture, TexturePool, Vec4, rotateTo
...
class MakeAToon(StateData.StateData):
    ...
    def load(self):
        ...
        self.guiCancelButton = DirectButton(parent=self.guiBottomBar, relief=None, image=(guiCancelUp,
         guiCancelDown,
         guiCancelUp,
         guiCancelDown), clickSound=loader.loadSfx('phase_3/audio/sfx/GUI_create_toon_back.ogg'), image_scale=halfButtonScale, image1_scale=halfButtonHoverScale, image2_scale=halfButtonHoverScale, pos=(-1.179, 0, -0.011), command=self.__handleCancel, text=('', TTLocalizer.MakeAToonCancel, TTLocalizer.MakeAToonCancel), text_font=ToontownGlobals.getInterfaceFont(), text_scale=TTLocalizer.MATguiCancelButton, text_pos=(0, 0.115), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1))
        ...
        self.guiLastButton = DirectButton(parent=self.guiBottomBar, relief=None, image=(guiNextUp,
         ...
         TTLocalizer.MakeAToonLast,
         ''), clickSound=loader.loadSfx('phase_3/audio/sfx/GUI_create_toon_back.ogg'), text_font=ToontownGlobals.getInterfaceFont(), text_scale=0.08, text_pos=(0, 0.115), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1))
        ...
        self.trackWalls = self.room2.find('**/colorWalls')
        self.trackWalls.setColor(0.8, 0.7, 1, 1)
        self.trackWalls.flattenStrong()
        self.trackProps = self.room2.find('**/genderProps')
    ...
    def unload(self):
        ...
        self.trackWalls.removeNode()
        self.trackProps.removeNode()
        del self.trackWalls
        del self.trackProps
    ...
    def enterTrackShop(self):
        self.shop = TRACKSHOP
        self.guiTopBar['text'] = TTLocalizer.PickTracksTitle
        self.guiTopBar['text_fg'] = (1, 0.92, 0.2, 1)
        self.guiTopBar['text_scale'] = TTLocalizer.MATenterClothesShop
        self.accept('TrackShop-done', self.__handleTrackShopDone)
        self.dropRoom(self.trackWalls, self.trackProps)
        self.toon.setScale(self.toonScale)
        self.toon.setPos(Point3(-4.01, -3.19, 0))
        if not self.progressing:
            self.toon.setHpr(self.toonHpr)
        self.trackShopOpening()
        self.ts.enter(self.toon)
        if TRACKSHOP not in self.shopsVisited:
            self.shopsVisited.append(TRACKSHOP)
		
    def exitTrackShop(self):
        self.squishRoom(self.trackWalls)
        self.squishProp(self.trackProps)
        self.ts.exit()
