...
class ShardPage(ShtikerPage.ShtikerPage):
    ...
    def firstLoadShard(self, buttonTuple):
        ...
        for i in xrange(len(curShardTuples)):
            shardId, name, difficulty, pop, invasionStatus, groupAvCount = curShardTuples[i]
            if shardId == actualShardId:
                ...
                self.reloadRightBrain(pop, name, difficulty, groupAvCount, shardId, buttonTuple)
    ...
    def joinGroup(self, group, shardId):
        ...
        self.acceptOnce('groupDialogDone', self.__handleGroupDialog, extraArgs=[canonicalHoodId, shardId])
        self.groupDialog = TTDialog.TTGlobalDialog(style=TTDialog.TwoChoice, text=TTLocalizer.GroupAskAccess % (hoodName[0], hoodName[-1], shardName), doneEvent='groupDialogDone')
    ...
    def reloadRightBrain(self, shardPop, shardName, shardDifficulty, groupAvCount, shardId, buttonTuple):
        ...
        difficultyText = self.getDistrictDifficulty(shardDifficulty)
        districtInfoNode = self.districtInfo.attachNewNode('district-info')
        self.districtStatusLabel = DirectLabel(parent=districtInfoNode, relief=None, pos=(0.4247, 0, 0.52), text=popText, text_scale=0.09, text_fg=Vec4(0, 0, 0, 1), textMayChange=1)
        self.districtDifficultyLabel = DirectLabel(parent=districtInfoNode, relief=None, pos=(0.4247, 0, 0.45), text=difficultyText, text_scale=0.06, text_fg=Vec4(0, 0, 0, 1), textMayChange=1)
    ...
    def getDistrictDifficulty(self, difficulty):
        return TTLocalizer.DistrictDifficulty.format(TTLocalizer.DistrictDifficultyNames[difficulty])
    ...
    def updateScrollList(self):
        ...
        for i in xrange(len(curShardTuples)):
            ...
            if buttonTuple == None:
                ...
                buttonTuple[1]['extraArgs'] = [pop, name, difficulty, groupAvCount, shardId, buttonTuple]
                buttonTuple[2]['command'] = self.reloadRightBrain
                buttonTuple[2]['extraArgs'] = [pop, name, difficulty, groupAvCount, shardId, buttonTuple]
