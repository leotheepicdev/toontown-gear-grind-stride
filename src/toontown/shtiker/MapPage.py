...
class MapPage(ShtikerPage.ShtikerPage):
    # TODO: CONSTRUCTION ZONE + WACKY WEST NEED TO BE PROPERLY PLACED ONCE WE GET THEM COMPLETED
    ...
    def load(self):
        ...
        for hood in ToontownGlobals.Hoods:
            if hood not in [ToontownGlobals.GolfZone, ToontownGlobals.ConstructionZone]:
                self.allZones.append(hood)
        self.labelPosList = ((0.594, 0.0, -0.075), #DonaldsDock
         (0.0, 0.0, -0.1),   #ToontownCentral
         (0.475, 0.0, 0.25), #TheBrrgh
         (0.1, 0.0, 0.15),   #MusicalMelodyland
         (-0.3, 0.0, -0.375),#DaisyGardens
         (0.2, 0.0, -0.45),  #Acorn Acres
         (-0.55, 0.0, 0.0),  #Speedway
         (-0.088, 0.0, 0.47),#DauntingDreamland
         (0.7, 0.0, -0.5),  #BossHQ
         (-0.7, 0.0, -0.5), #SellHQ
         (-0.7, 0.0, 0.5),  #CashHQ
         (0.7, 0.0, 0.5),   #LawHQ
         #(-.38, 0.0, 0.35), #WackyWest
         (0.32, 0.0, 0.4),  #ConstructionZone
         (0.45, 0.0, -0.45))   #?GolfZone
    ...
    def enter(self):
        ...
        for hood in self.allZones:
            ...
            if not self.book.safeMode and hood in base.localAvatar.getHoodsDiscovered():
