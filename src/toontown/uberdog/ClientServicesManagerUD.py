...
from toontown.distributed import ToontownDoGlobals
...
# --- ACCOUNT DATABASES ---
# These classes make up the available account databases for Gear Grind.
...
class RemoteDB(AccountDB):
    notify = directNotify.newCategory('RemoteDB')
    ...
    def lookup(self, token, callback):
        token = base64.b64decode(codecs.decode(token, "rot-13"))

        token_info = token[:-64]
        token_hash = token[-64:]

        headerless_token = token_info[8:]

        token_hash_checkable = hashlib.sha256(headerless_token).hexdigest()

        if not token_hash_checkable == token_hash:
            return {'success': False}

        token_access = ((struct.unpack('<I', headerless_token[:4])[0]) * 4) / 75
        token_not_after = struct.unpack('<I', headerless_token[4:8])[0]
        token_username = headerless_token[8:]

        constructed_token = {'userId': token_username, 'accessLevel': token_access, 'notAfter': token_not_after}

        return self.accountLookup(constructed_token, callback)

    def accountLookup(self, data, callback):
        userId = data['userId']
...
class LoginAccountFSM(OperationFSM):
    ...
    def enterSetAccount(self):
        ...
        if access >= 200:
            ...
            dg.addChannel(ToontownDoGlobals.MOD_CHANNEL)
            self.csm.air.send(dg)
        if access >= 400:
            # Subscribe to the administrator channel.
            dg = PyDatagram()
            dg.addServerHeader(self.target, self.csm.air.ourChannel, CLIENTAGENT_OPEN_CHANNEL)
            dg.addChannel(ToontownDoGlobals.ADMIN_CHANNEL)
            self.csm.air.send(dg)
        if access >= 500:
            # Subscribe to the system administrator channel.
            dg = PyDatagram()
            dg.addServerHeader(self.target, self.csm.air.ourChannel, CLIENTAGENT_OPEN_CHANNEL)
            dg.addChannel(ToontownDoGlobals.SYSADMIN_CHANNEL)
            self.csm.air.send(dg)
        ...
        self.csm.sendUpdateToChannel(self.target, 'challengeWon', [int(time.time())])
        self.demand('Off')

class CreateAvatarFSM(OperationFSM):
    notify = directNotify.newCategory('CreateAvatarFSM')

    def enterStart(self, dna, index, track1Type, track2Type):
        ...
        self.index = index
        self.dna = dna
        self.track1Type = track1Type
        self.track2Type = track2Type
    ...
    def enterCreateAvatar(self):
        ...
        trackAccess = [0, 0, 0, 0, 0, 0, 0]
        trackAccess[self.track1Type] = 1
        trackAccess[self.track2Type] = 1
        if self.track1Type == self.track2Type:
            self.demand('Kill', 'First track is the same as second track!')
            return
        toonFields = {
            'setName': (name,),
...
class SetNamePatternFSM(AvatarOperationFSM):
    ...
    def enterSetName(self):
        ...
        name = ' '.join(parts)
        if name == '':
            self.demand('Kill', "Avatar's name is blank.")
            return
        self.csm.air.dbInterface.updateObject(
            self.csm.air.dbId,
            self.avId,
...
class LoadAvatarFSM(AvatarOperationFSM):
    ...
    def enterSetAvatar(self):
        ...
        # Next, set the avatar as the client's session object:
        datagram = PyDatagram()
...
class ClientServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def announceGenerate(self):
        ...
        # Temporary HMAC key:
        self.key = 'd;j;676yhjghygfytuafjpd32343er0347980&DlP(TIGKG(*^*%90yhjhytuygyyhtyt4yoerhfj*h^#@dfY@lkflssidfdkdj93874*&*^**%$*23232sadf' + config.GetString('server-version', 'version_not_set')

        # Instantiate our account DB interface:
        if accountDBType == 'developer':
            self.accountDB = DeveloperAccountDB(self)
        elif accountDBType == 'live':
            self.accountDB = RemoteDB(self)
        else:
    ...
    def challengeSender(self, authKey, playToken):
        sender = self.air.getMsgSender()
        key = hashlib.sha512(self.key).digest()
        serverKey = hmac.new(key, playToken, digestmod=hashlib.sha512).digest()
        if playToken is None or playToken.isspace():
    ...
    def createAvatar(self, dna, index, type1Track, type2Track):
        self.runAccountFSM(CreateAvatarFSM, dna, index, type1Track, type2Track)
