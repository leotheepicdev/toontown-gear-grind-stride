...
class ChatManager(DirectObject.DirectObject):
    ...
    CHAT_KEY = base.getKey('TYPE_CHAT')
    ...
    def sendSCChatMessage(self, msgIndex, CFType=0):
    ...
    def enterMainMenu(self):
        ...
        if base.cr.wantTypedChat():
            self.acceptOnce(self.CHAT_KEY, self.fsm.request, ['normalChat'])
    ...
    def enterWhisper(self, avatarName, avatarId):
        ...
        if avatarUnderstandable and online:
            self.acceptOnce(self.CHAT_KEY, self.fsm.request, ['whisperChat', [avatarName, avatarId]])
    ...
    def enterWhisperChat(self, avatarName, avatarId):
        base.localAvatar.controlManager.ignoreControls()
        result = self.chatInputNormal.activateByData(avatarId)
        return result

    def exitWhisperChat(self):
        base.localAvatar.controlManager.listenToControls()
    ...
    def updateChatKey(self):
        self.ignore(self.CHAT_KEY)
        self.chatInputNormal.chatEntry['backgroundFocus'] = 0
        self.CHAT_KEY = base.getKey('TYPE_CHAT')
