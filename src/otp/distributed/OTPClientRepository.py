...
from panda3d.core import Connection, Datagram, DatagramIterator, Filename, NodePath, Notify, getModelPath
...
from toontown.avatar.DistributedPlayer import DistributedPlayer
from toontown.distributed.ToontownDoGlobals import *
from toontown.distributed.TelemetryLimiter import TelemetryLimiter
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def enterConnect(self, serverList):
        ...
        if self.toontownTitleScreen:
            self.toontownTitleScreen.setLabelText(OTPLocalizer.CRConnecting)
        else:
    ...
    def enterLogin(self):
        ...
        if self.toontownTitleScreen:
            self.toontownTitleScreen.setLabelText(OTPLocalizer.CRAuthenticating)
        else:
            dialogClass = ToontownGlobals.getGlobalDialogClass()
        ...
        self.csm.requestChallenge(self.loginDoneEvent)
    ...
    def enterWaitForGameList(self):
        self.gameDoDirectory = self.addTaggedInterest(self.GameGlobalsId, ZONE_ID_MANAGEMENT, self.ITAG_PERM, 'game directory', event='GameList_Complete')
    ...
    def enterWaitForShardList(self):
        if not self.isValidInterestHandle(self.shardListHandle):
            self.shardListHandle = self.addTaggedInterest(self.GameGlobalsId, ZONE_ID_DISTRICTS, self.ITAG_PERM, 'LocalShardList', event='ShardList_Complete')
    ...
    def enterNoConnection(self):
        ...
        reconnect = 1
        if self.bootedIndex in (152, 127, 124, 103, 105, 106):
            reconnect = 0
        ...
        self.accept('lostConnectionAck', self.__handleLostConnectionAck, extraArgs=[reconnect])
        self.notify.warning('Lost connection to server. Notifying user.')

    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def __handleLostConnectionAck(self, reconnect):
        if not reconnect:
            self.loginFSM.request('shutdown')
            return
    ...
    def getStartingDistrict(self):
        ...
        if preferred:
            for shard in self.activeDistrictMap.values():
                if shard.available and shard.name == preferred and shard.population < maxPop:
                    return shard

        for shard in self.activeDistrictMap.values():
            if shard.available and shard.population < maxPop:
                district = shard
                maxPop = district.population

        if district:
            self.notify.debug('chose %s: pop %s' % (district.name, district.population))
    ...
    def listActiveShards(self):
        ...
        for s in self.activeDistrictMap.values():
            if s.available:
                list.append((s.doId, s.name, s.difficulty, s.population, s.invasionStatus, s.groupAvCount))
